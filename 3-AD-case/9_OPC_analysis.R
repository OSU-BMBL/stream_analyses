...
# Parameters
ct <- "OPC"
cds <- qs::qread(paste0(R.dir, "opc_cds.qsave"))
...
rownames(ht.mat) <- sapply(cts.en.regs, "[[", "TF")[as.numeric(rownames(ht.mat))] %>% 
  ...
  tolower %>% capitalize
p.heatmap <- get_simple_heatmap(mat = ht.mat[, -c(13, 14, 18)], name = "Z score of regulatory strength", 
                   bottom = -1.4, interium = 0.0, up = 1.2, labels = c("-1.3", "1.8"),
                   low = "white", middle = "#a6f7e9", high = "#00FFD1")
save_image(p = p.heatmap, paste0(image.dir, ct, "_heatmap.png"), 
...
# Plotting for down
down.dot <- jaccard_pseudotime(x = gene.mat[down.genes[2],], y = enh.mat[names(down.enh)[down.id],], 
                               eq.x = 1.0, eq.y = 0.1, 
                   rsquare.x = 1.0, rsquare.y = 0.25,
...
save.image(paste0(scratch.dir, "28_OPC_analysis.RData"))

...
# Parameters
ct <- "MG"
cds <- qs::qread(paste0(R.dir, "mg_cds.qsave"))
...
rownames(ht.mat) <- sapply(cts.en.regs, "[[", "TF")[as.numeric(rownames(ht.mat))] %>% 
  ...
  tolower %>% capitalize
p.heatmap <- get_simple_heatmap(mat = ht.mat[, -c(7, 17:19)], name = "Z score of regulatory strength", 
                   bottom = -1.5, interium = 0, up = 1.0, labels = c("-1.2", "1.4"),
                   low = "white", middle = "#fac5c5", high = "#FF9494")
...
save.image(paste0(scratch.dir, "27_MG_analysis.RData"))

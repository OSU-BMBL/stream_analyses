...
# Parameters
ct <- "AG"
cds <- qs::qread(paste0(R.dir, "ag_cds.qsave"))
...
rownames(ht.mat) <- sapply(cts.en.regs, "[[", "TF")[as.numeric(rownames(ht.mat))] %>% 
  ...
  tolower %>% capitalize
p.heatmap <- get_simple_heatmap(mat = ht.mat[, -c(2, 5)], name = "Z score of regulatory strength", 
                   bottom = -2.0, interium = 0.0, up = 0.5, labels = c("-2.3", "1.3"),
                   low = "white", middle = "#cf4455", high = "#D2001A")
...
save.image(paste0(scratch.dir, "26_AG_analysis.RData"))

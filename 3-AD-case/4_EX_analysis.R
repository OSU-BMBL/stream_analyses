...
# Parameters
ct <- "EX"
cds <- qs::qread(paste0(R.dir, "ex_cds.qsave"))
pt.genes <- qs::qread(paste0(R.dir, "EX_pt_genes.qsave"))
...
# Get genes changing as a function of pseudotime
genes <- Reduce("union", lapply(cts.en.regs, function(x) {
  unique(x$links$gene)
}))
length(genes)
enhs <- Reduce("union", lapply(cts.en.regs, function(x) {
  unique(GRangesToString(x$links))
}))
length(enhs)
qs::qsave(cds[genes,], paste0(R.dir, ct, "_subset_cds.qsave"))
pt.genes <- graph_test(cds[genes,], neighbor_graph = "principal_graph", cores = 4)
sig.genes <- pt.genes[pt.genes[, 12] < 0.05,]
dim(sig.genes)



# Get enhancers changing as a function of pseudotime
pt.cells <- pseudotime(cds) %>% sort
head(pt.cells)
tail(pt.cells)
length(pt.cells)
enh.mat <- GetAssayData(obj, assay = "ATAC", slot = "data")[enhs, names(pt.cells)]
gene.mat <- GetAssayData(obj, assay = "RNA", slot = "data")[genes, names(pt.cells)]
dim(enh.mat)
dim(gene.mat)
pt.enhs <- monocle3_TI_atac(mat = enh.mat, pseudotime.annot = pt.cells, nBins = 20, 
                              binarize = F)$da.regions
dim(pt.enhs)
sig.enhs <- pt.enhs[pt.enhs$q_value < 0.05,]
dim(sig.enhs)



# Compute correlation between gene expression and pseudotime
require(pbmcapply)
cor.genes <- cbind(sig.genes, cor = pbmclapply(1:nrow(sig.genes), function(i) {
  cor.test(gene.mat[rownames(sig.genes)[i], names(pt.cells)], 
           pt.cells, method = "spearman")$estimate
}, mc.cores = detectCores()) %>% unlist)
cor.genes <- cor.genes[order(cor.genes$cor, decreasing = T),]
dim(cor.genes)
range(cor.genes$cor)



# Compute correlation between enhancer accessibility and pseudotime
enh.bin <- apply(enh.mat, 1, function(x) {
  as.numeric(x > 0)
}) %>% t
colnames(enh.bin) <- colnames(enh.mat)
cor.enhs <- cbind(sig.enhs, cor = pbmclapply(1:nrow(sig.enhs), function(i) {
  cor.test(enh.bin[sig.enhs$site_name[i], names(pt.cells)] %>% as.numeric, 
           pt.cells, method = "spearman")$estimate
}, mc.cores = detectCores()) %>% unlist)
cor.enhs <- cor.enhs[order(cor.enhs$cor, decreasing = T),]
dim(cor.enhs)
range(cor.enhs$cor) # low correlation
...
rownames(ht.mat) <- sapply(cts.en.regs, "[[", "TF")[as.numeric(rownames(ht.mat))] %>% 
  ...
  tolower %>% capitalize
range(pt.cells)
p.heatmap <- get_simple_heatmap(mat = ht.mat[, -9], name = "Z score of regulatory strength", 
                   bottom = -1.5, interium = -0.0, up = 0.5, labels = c("-1.6", "1.0"),
                   low = "white", middle = "#727db8", high = "#25316D")
save_image(p = p.heatmap, paste0(image.dir, "EX_heatmap.png"), 
...
save.image(paste0(scratch.dir, "23_EX_analysis.RData"))

...
# Parameters
ct <- "Oligo"
cds <- qs::qread(paste0(R.dir, "oligo_cds.qsave"))
...
rownames(ht.mat) <- sapply(cts.en.regs, "[[", "TF")[as.numeric(rownames(ht.mat))] %>% 
  ...
  tolower %>% capitalize
p.heatmap <- get_simple_heatmap(mat = ht.mat, name = "Z score of regulatory strength", 
                   bottom = -1.5, interium = 0, up = 1.0, labels = c("-1.8", "1.5"),
                   low = "white", middle = "#cde6b3", high = "#B6E388")
...
save.image(paste0(scratch.dir, "25_Oligo_analysis.RData"))
